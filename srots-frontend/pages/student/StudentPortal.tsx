...
//                                       <input className="w-full border border-gray-300 p-2.5 rounded-lg bg-white text-gray-900 outline-none text-sm" placeholder="Full Address Line" value={wizProfile.currentAddress?.addressLine1} onChange={e => setWizProfile({...wizProfile, currentAddress: {...wizProfile.currentAddress, addressLine1: e.target.value}})}/>
...
//                                           <input className="w-full border border-gray-300 p-2.5 rounded-lg bg-white text-gray-900 outline-none text-sm" placeholder="Pincode" value={wizProfile.currentAddress?.zip} onChange={e => setWizProfile({...wizProfile, currentAddress: {...wizProfile.currentAddress, zip: e.target.value}})}/>
...
//                                   </div>
//                                   <div className="space-y-3">
//                                       <input className="w-full border border-gray-300 p-2.5 rounded-lg bg-white text-gray-900 outline-none text-sm" placeholder="Full Address Line" value={wizProfile.permanentAddress?.addressLine1} onChange={e => setWizProfile({...wizProfile, permanentAddress: {...wizProfile.permanentAddress, addressLine1: e.target.value}})}/>
//                                       <div className="grid grid-cols-2 gap-3">
...
//                                           <input className="w-full border border-gray-300 p-2.5 rounded-lg bg-white text-gray-900 outline-none text-sm" placeholder="Pincode" value={wizProfile.permanentAddress?.zip} onChange={e => setWizProfile({...wizProfile, permanentAddress: {...wizProfile.permanentAddress, zip: e.target.value}})}/>
...
export const StudentPortal: React.FC<StudentPortalProps> = ({ view, student, onUpdateUser }) => {
    ...
    useEffect(() => {
        const fetchFullStudent = async () => {
            // Safety guard: ensure student ID is present to avoid 400 Bad Request
            ...
            if (!student.profile) {
                try {
                    ...
                    console.error("Failed to fetch student 360 data:", error);
                }
            }
        };
        fetchFullStudent();
