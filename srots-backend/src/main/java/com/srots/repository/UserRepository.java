...
public interface UserRepository extends JpaRepository<User, String> {
	...
	@Query("SELECT u FROM User u LEFT JOIN FETCH u.studentProfile LEFT JOIN FETCH u.educationRecords WHERE u.college.id = :cid AND u.role = :role")
	List<User> findByCollegeIdAndRoleWithProfile(@Param("cid") String collegeId, @Param("role") User.Role role);
	...
	boolean existsByCollegeIdAndRollNumber(@Param("collegeId") String collegeId,
			@Param("rollNumber") String rollNumber);

	// This helps during Update: find a student with this roll in this college who
	...
	boolean existsByCollegeIdAndRollNumberAndIdNot(@Param("collegeId") String collegeId,
			@Param("rollNumber") String rollNumber, @Param("currentUserId") String currentUserId);
